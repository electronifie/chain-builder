...
CallQueue.prototype.start = function (initialResult, cb) {
  this._chainStartMemo = this._logger.chainStart(this._parentQueue && this._parentQueue._chainStartMemo);
...
CallQueue.prototype._runCall = function (callDescriptor, cb) {
  ...
  var done = function (error, result) {
    this._logger.callEnd(callStartMemo, callDescriptor, error, result);
    cb.call(this, error, result);
  }.bind(this);

  try {
    this._runSafely(function () {
      callStartMemo = this._logger.callStart(this._chainStartMemo, callDescriptor);
      var args;

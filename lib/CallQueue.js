...
CallQueue.prototype._runCall = function (callDescriptor, cb) {
  ...
  var done = function (error, result) {
    ...
    if (method.$beginSubchain) {
      // ignore
    } else if (method.$endSubchain) {
      log.subchainFinished(this._depth, start, end, method.$endSubchain, error, result);
    } else {
  ...
  try {
    this._runSafely(function () {
      if (method.$beginSubchain) {
        ...
        log.subchainStarted(this._depth, method.$endSubchain);
      } else {

 ...
 * @property {String}    stackEntry  - the origin of the call
 */
 ...
 * @param {Boolean}                   options.enableStack - gather stack information
...
var CallQueue = function (options) {
  ...
  this._enableStack = !!options.enableStack;
...
CallQueue.prototype.add = function (methodName, args, skipOnError) {
  ...
  var stack = this._enableStack ? cleanStack() : [];
  ...
  if (this._subqueue && !createdSubqueue) {
    this._subqueue.add(methodName, args, skipOnError, stack);
  } else {
    this._queue.push({
      method: methodName,
      subqueue: subqueue,
...
CallQueue.prototype._runCall = function (callDescriptor, cb) {
  ...
  var done = function (error, result) {
    if (callStartMemo) this._logger.callEnd(callStartMemo, callDescriptor, this._enableStack ? cleanStack() : [], error, result);
  ...
  try {
    this._runSafely(function () {
      ...
      this._context.setCallDescriptor(callDescriptor);

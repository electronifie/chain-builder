 ...
 * @param {LogHandler[]}       logHandlers    - methods to receive logging events
 * @param {{
 *   enableStack: Boolean
 * }}                          globalOpts
 * @returns {ChainConstructor}
 */
module.exports = function (methods, contextMethods, logHandlers, globalOpts) {
  var Context = contextFactory(contextMethods);
  var logger = new Logger(logHandlers);
  ...
  Chain.prototype._newCallQueue = function () {
    return new CallQueue({
      ...
      Chain: Chain,
      Context: Context,
      enableStack: globalOpts.enableStack
    });
  };
